...
for env in shared staging prod; do
    ...
    sed -i "s/default/${AWS_PROFILE}/g" "config/${env}.tfbackend"
...
for env in shared staging prod; do
    ...
    # Replace placeholders in the copied file
    sed -i "s/YOUR_UNIQUE_PREFIX/${PREFIX}/g" "config/${env}.yaml"
    sed -i "s/us-east-1/${REGION}/g" "config/${env}.yaml"
    sed -i "s/your-aws-profile-name/${AWS_PROFILE}/g" "config/${env}.yaml"
    
    # Update GitHub URL if provided (shared environment only)
    if [[ -n "$GITHUB_URL" && "$env" == "shared" ]]; then
        sed -i "s|https://github.com/your-username/your-repo-name|${GITHUB_URL}|g" "config/${env}.yaml"
        sed -i "s/# GitHub repository URL for App Runner source connection/# GitHub repository URL for App Runner source connection (provided via --github-url)/g" "config/${env}.yaml"
    fi
    
    # Add generation comment
    sed -i "1i# Generated by bootstrap-terraform-backend.sh on $(date)" "config/${env}.yaml"
done
...
echo "   - config/shared.yaml"
echo "   - config/staging.yaml"
...
echo "🚀 Next Steps:"
echo "   1. Update GitHub repository URL in config/shared.yaml (if not provided)"

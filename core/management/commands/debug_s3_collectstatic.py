...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Django Version: {importlib.import_module('django').__version__}")

        # Static files settings
        self.stdout.write("\nStatic Files Settings:")
        self.stdout.write(f"STATICFILES_STORAGE: {getattr(settings, 'STATICFILES_STORAGE', 'Not set')}")
        ...
        self.stdout.write(f"AWS_ACCESS_KEY_ID: {os.environ.get('AWS_ACCESS_KEY_ID', 'Not set')[:4]}**** (masked)")
        self.stdout.write(f"AWS_SECRET_ACCESS_KEY: {'**** (masked)' if os.environ.get('AWS_SECRET_ACCESS_KEY') else 'Not set'}")
        ...
        try:
            ...
            bucket_name = getattr(settings, "AWS_STORAGE_BUCKET_NAME", os.environ.get("AWS_STORAGE_BUCKET_NAME"))
            if bucket_name:
                # Try listing objects to verify credentials and bucket access
                storage.connection.meta.client.list_objects_v2(Bucket=bucket_name, MaxKeys=1)
                self.stdout.write(f"S3 Connection: Successful (able to list objects in bucket {bucket_name})")
            else:
                self.stdout.write("S3 Connection: Skipped (AWS_STORAGE_BUCKET_NAME not set)")

...
class EmailTemplateTest(TestCase):
    ...
    def test_admin_notification_email_templates(self):
        context = {
            'user': self.user,
            'admin_url': 'http://testserver/admin/core/user/'
        }
...
class EmailIntegrationTest(TestCase):
    ...
    def test_registration_sends_admin_notification(self, mock_send_mail):
        ...
        with patch('core.fields.requests.post') as mock_captcha:
            ...
            self.assertIn('New Ministry Leader Registration Request', call_args[1]['subject'])
    ...
    def test_email_template_context_variables(self):
        ...
        admin_message = render_to_string('core/emails/admin_notification_body.txt', admin_context)
        ...
        approval_message = render_to_string('core/emails/approval_body.txt', approval_context)

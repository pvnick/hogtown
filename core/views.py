...
def parish_detail(request, parish_id):
    ...
    return render(request, 'core/parish_detail.html', {
        'parish': parish,
...
def ministry_detail(request, ministry_id):
    ...
    return render(request, 'core/ministry_detail.html', {
        'ministry': ministry,
        'events': events
    })
...
def get_calendar_events(request):
    ...
    recurring_events = Event.objects.filter(
        ...
        models.Q(series_end_date__isnull=True) | models.Q(series_end_date__gte=start)
    ).select_related('associated_ministry__associated_parish')
    ...
    for event in recurring_events:
        if event.recurrence_rule:
            try:
                ...
                dtstart = datetime.combine(event.series_start_date, event.start_time_of_day)
                ...
                exceptions = EventException.objects.filter(
                    event=event,
                    original_occurrence_date__range=[start, end]
                )
                exception_dict = {exc.original_occurrence_date: exc for exc in exceptions}
                ...
                for occurrence in occurrences:
                    ...
                    # Normal occurrence
                    start_dt = datetime.combine(occurrence_date, event.start_time_of_day)
...
def ministry_portal(request):
    ...
    return render(request, 'core/ministry_portal.html', {
        'ministries': user_ministries
    })
...
class EventCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['associated_ministry', 'title', 'description', 'location', 'is_recurring',
              'start_datetime', 'end_datetime', 'series_start_date', 'series_end_date',
              'start_time_of_day', 'end_time_of_day', 'recurrence_rule']
...
def event_occurrence_action(request, event_id, occurrence_date):
    ...
    event = get_object_or_404(Event, pk=event_id, associated_ministry__owner_user=request.user)
    
    if not event.is_recurring:
        return JsonResponse({'error': 'This action is only available for recurring events'}, status=400)
    ...
    if request.method == 'POST':
        ...
        if action == 'cancel':
            ...
            if not new_start or not new_end:
                return JsonResponse({'error': 'New start and end times required'}, status=400)

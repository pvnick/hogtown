...
class MinistryLeaderRegistrationForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ('username', 'email', 'full_name', 'associated_parish', 'requested_ministry_details', 'password1', 'password2')
    ...
    def save(self, commit=True):
        ...
        user.requested_ministry_details = self.cleaned_data['requested_ministry_details']
        user.role = 'leader'
        user.status = 'pending'
        if commit:
            user.save()

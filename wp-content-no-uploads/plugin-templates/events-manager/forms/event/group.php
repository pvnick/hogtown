...
/*
 * Changes: 
 * - Allow selecting a group via the group_id url parameter
 */
global $EM_Event;
...
if( count($user_groups) > 0 ){ 
	...
	<select name="group_id">
		...
		foreach($user_groups as $BP_Group){
			?>
			<option value="<?php echo $BP_Group->id; ?>" <?php echo ($BP_Group->id == $EM_Event->group_id || $BP_Group->id == intval($_REQUEST['group_id'])) ? 'selected="selected"':''; ?>><?php echo $BP_Group->name; ?></option>
			<?php
		} 
		?>
	</select>

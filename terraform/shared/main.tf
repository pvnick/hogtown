...
# Domain is registered externally but DNS is managed by AWS Route 53
resource "aws_route53_zone" "main" {
  name = local.config.domain_name

  tags = {
    Name    = "${local.config.project_name}-hosted-zone"
    Project = local.config.project_name
  }
}

# ACM Certificate for custom domain (including wildcard for subdomains)
resource "aws_acm_certificate" "main" {
  ...
  tags = {
    Name    = "${local.config.project_name}-ssl-certificate"
    Project = local.config.project_name
  }

...
def lambda_handler(event, context):
    ...
    try:
        ...
        for db_name in environment_databases:
            ...
            if not cursor.fetchone():
                cursor.execute(
                    f'CREATE USER "{user_name}" WITH PASSWORD %s', (password,)
                )
                logger.info(f"Created user: {user_name}")
            else:
                # Update password for existing user
                cursor.execute(
                    f'ALTER USER "{user_name}" WITH PASSWORD %s', (password,)
                )
            ...
            db_cursor.execute(
                f'GRANT CONNECT, CREATE, TEMPORARY ON DATABASE "{db_name}" TO "{user_name}"'
            )
            ...
            db_cursor.execute(
                f'GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO "{user_name}"'
            )
            db_cursor.execute(
                f'GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO "{user_name}"'
            )
